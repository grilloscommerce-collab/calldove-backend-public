...
app.post('/api/auth/send-verification', async (req, res) => {
  try {
    ...
    await 
twilioClient.verify.v2.services('VAfda39f88eaabea55df09f201fa193108').verifications.create({ 
...
to: phone, code: code });
    if (verification.status === 'approved') {
      const db = await connectDB();
      await db.collection('users').updateOne({ phone }, { $set: { 
verified: true } });
      res.json({ success: true });
    } else {
...
process.env.TWILIO_TWIML_APP_SID, incomingAllow: true });
    const token = new AccessToken(process.env.TWILIO_ACCOUNT_SID, 
process.env.TWILIO_API_KEY, process.env.TWILIO_API_SECRET, { identity: 
userId });
...
${targetPhone}`);
    ...
    activeConferences.set(conferenceName, { userPhone, targetPhone, 
userLanguage: sourceLanguage, targetLanguage: 'auto', createdAt: new 
Date() });
    const userCall = await twilioClient.calls.create({
      url: 
`${BASE_URL}/connect-user?conference=${conferenceName}&lang=${sourceLanguage}`,
      to: userPhone, from: process.env.TWILIO_PHONE_NUMBER,
      statusCallback: `${BASE_URL}/call-status`,
      statusCallbackEvent: ['answered', 'completed']
    });
    console.log(`✅ User call initiated: ${userCall.sid}`);
    res.json({ success: true, callSid: userCall.sid, conferenceName, 
...
encoding="UTF-8"?><Response><Say>Connecting your call with 
translation.</Say><Dial><Conference startConferenceOnEnter="true" 
endConferenceOnExit="true" 
statusCallback="${BASE_URL}/conference-status?conference=${conferenceName}" 
statusCallbackEvent="start,end,join,leave">${conferenceName}</Conference></Dial></Response>`;
    res.type('text/xml');
    res.send(twiml);
    setTimeout(async () => {
...
`${BASE_URL}/connect-target?conference=${conferenceName}&lang=${confData.targetLanguage}`,
          to: confData.targetPhone, from: process.env.TWILIO_PHONE_NUMBER
        });
      }
    }, 2000);
  } catch (error) {
...
${conferenceName}`);
    const twiml = `<?xml version="1.0" 
encoding="UTF-8"?><Response><Dial><Conference 
startConferenceOnEnter="false" 
endConferenceOnExit="true">${conferenceName}</Conference></Dial></Response>`;
    res.type('text/xml');
    res.send(twiml);
  } catch (error) {
...
${target}`);
    const twimlResponse = `<?xml version="1.0" 
encoding="UTF-8"?><Response><Say>Translation ready. After the beep, speak 
...
Buffer.from(`${process.env.TWILIO_ACCOUNT_SID}:${process.env.TWILIO_AUTH_TOKEN}`).toString('base64');
      https.get(downloadUrl, { headers: { 'Authorization': `Basic ${auth}` 
} }, (response) => {
        response.pipe(file);
...
fs.createReadStream(tmpFile), model: 'whisper-1' });
    fs.unlinkSync(tmpFile);
    if (!transcription.text || transcription.text.trim().length === 0) {
      console.log('No speech detected');
      const twiml = `<?xml version="1.0" 
encoding="UTF-8"?><Response><Say>No speech detected. Please speak clearly 
...
"${transcription.text}"`);
    const translation = await openai.chat.completions.create({
      model: 'gpt-4',
      messages: [{ role: 'system', content: `Translate from 
${languageMap[detectedLang]?.name || 'the source language'} to 
${languageMap[target].name}. Output ONLY the translation, nothing else.` 
    ...
    const twiml = `<?xml version="1.0" 
encoding="UTF-8"?><Response><Say>${translatedText.replace(/&/g, 
'&amp;').replace(/</g, '&lt;').replace(/>/g, '&gt;')}</Say><Say>Speak 
...
participants: userId }).sort({ lastMessageAt: -1 }).toArray();
...
app.get('/api/messages/:conversationId', async (req, res) => {
  try {
    ...
    const db = await connectDB();
    const messages = await db.collection('messages').find({ conversationId 
...
participants: [senderId, recipientId], createdAt: new Date(), 
lastMessageAt: new Date() });
      conversation = { _id: result.insertedId };
    }
    const recipient = await db.collection('users').findOne({ _id: new 
...
${recipientLanguage}`);
    ...
    if (senderLanguage !== recipientLanguage) {
      const translation = await openai.chat.completions.create({
        model: 'gpt-4',
        messages: [{ role: 'system', content: `Translate from 
${languageMap[senderLanguage].name} to 
${languageMap[recipientLanguage].name}. Output ONLY the translation, 
maintain the tone and style.` }, { role: 'user', content: text }],
        temperature: 0.3
      });
...
}, { $set: { lastMessageAt: new Date(), lastMessage: translatedText } });
    console.log('✅ Message sent and translated successfully!');
    res.json({ success: true, message: { ...message, _id: 
...
app.post('/api/messages/read', async (req, res) => {
  try {
    ...
    const db = await connectDB();
    await db.collection('messages').updateMany({ conversationId, 
recipientId: userId, read: false }, { $set: { read: true } });
    res.json({ success: true });
  } catch (error) {
...
role: 'user', content: text }],
    ...
    const message = {
      _id: Date.now().toString(), senderPhone, recipientPhone,
      originalText: text, originalLanguage: senderLanguage,
      translatedText, translatedLanguage: targetLanguage,
